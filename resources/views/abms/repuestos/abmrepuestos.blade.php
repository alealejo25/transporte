...
<script>
   $(document).ready(function () {
    ...
    $('#repuestoModal').on('show.bs.modal', function () {
        cargarMarcas(); // Llama a la función para cargar las marcas
    });
    ...
    // Función para cargar las marcas en el combo
    function cargarMarcas() {
    	 
        const combo = $('#marcarepuestos_id');
        $.get('/abms/repuestos/cargarmarcas', function (data) {
            combo.empty(); // Vaciar el combo para evitar duplicados
            combo.append('<option value="" disabled selected>Seleccione una marca</option>'); // Opción predeterminada
            data.forEach(function (marca) {
                combo.append(`<option value="${marca.id}">${marca.nombre}</option>`);
            });
             console.log('Opciones cargadas:', combo.html()); // Muestra el contenido del combo
        }).fail(function () {
            alert('Error al cargar las marcas');
        });
    }

    // Función para cargar los repuestos en la tabla
            ...
            data.forEach(function (repuesto) {
                tableBody.append(`
                    <tr>
                        ...
                        <td>${repuesto.cantidad_lnf}</td>
                        <td>${repuesto.cantidad_leagas}</td>
                        <td>${repuesto.cantidad_malebo}</td>
